...
export default async function FirmBaselinesPage({
  ...
  // Gestión operativa solo para OPERATOR (change firm-client-portal: el
  // portal del cliente queda reducido a 4 bloques). El operator llega desde
  // la pestaña Baselines del detalle de firma con ?firmId=<id>.
  if (session.user.role === "FIRM_ADMIN") redirect("/firm");
  if (session.user.role !== "OPERATOR") redirect("/login");
  if (!queryFirmId) redirect("/operator");
  const firmId: string = queryFirmId;
