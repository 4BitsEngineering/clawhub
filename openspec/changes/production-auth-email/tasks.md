...
- [x] 1.1 Verificar el dominio `iaofi.com` en Resend (SPF/DKIM) — hecho (dominio verificado)
- [ ] 1.2 Definir `RESEND_API_KEY` y `RESEND_FROM=AI-Office <info@iaofi.com>` en `.env` local y en las variables de entorno de Vercel (preview y producción)

## 2. Magic link real

- [x] 2.1 Unificar el fallback de remitente en `src/lib/mailer.ts` a `AI-Office <info@iaofi.com>`
